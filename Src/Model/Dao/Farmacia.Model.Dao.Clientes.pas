...
begin
  Query.ParamByName('pCOD_CLIENTE').AsInteger     := Cadastro.GetCodigo;
  Query.ParamByName('pNOM_CLIENTE').AsString      := Cadastro.GetNome;
  Query.ParamByName('pIND_DOCUMENTO').AsInteger   := Cadastro.GetIndDocumento;
  Query.ParamByName('pDOC_CLIENTE').AsString      := Cadastro.GetDocumento;
  Query.ParamByName('pNUM_CEP').AsString          := Cadastro.GetCep;
  Query.ParamByName('pEND_LOGRADOURO').AsString   := Cadastro.GetLogradouro;
  Query.ParamByName('pNUM_ENDERECO').AsInteger    := Cadastro.GetNumEndereco;
  Query.ParamByName('pNOM_BAIRRO').AsString       := Cadastro.GetBairro;
  Query.ParamByName('pNOM_CIDADE').AsString       := Cadastro.GetCidade;
  Query.ParamByName('pSIG_UF').AsString           := Cadastro.GetUf;
  Query.ParamByName('pEND_COMPLEMENTO').AsString  := Cadastro.GetComplemento;
  Query.ParamByName('pEND_ELETRONICO').AsString   := Cadastro.GetEndEletronico;
  Query.ParamByName('pTEL_FIXO').AsString         := Cadastro.GetTelFixo;
  Query.ParamByName('pTEL_FAX').AsString          := Cadastro.GetFax;
  Query.ParamByName('pTEL_CELULAR').AsString      := Cadastro.GetCelular;
  Query.ParamByName('pCOD_USUARIO_ALT').AsInteger := Cadastro.GetCodigoUsuario;
  Query.ParamByName('pDAT_CADASTRO').AsDate       := Cadastro.GetDataCadastro;
  Query.ParamByName('pDAT_ALTERACAO').AsDate      := Cadastro.GetDataAlteracao;
end;
...
begin
  ...
  Query.SQL.Clear;
  Query.SQL.Add('UPDATE FARMACIA.CLIENTES SET');
  Query.SQL.Add('  NOM_CLIENTE     = :pNOM_CLIENTE,');
  Query.SQL.Add('  IND_DOCUMENTO   = :pIND_DOCUMENTO,');
  Query.SQL.Add('  DOC_CLIENTE     = :pDOC_CLIENTE,');
  Query.SQL.Add('  NUM_CEP         = :pNUM_CEP,');
  Query.SQL.Add('  END_LOGRADOURO  = :pEND_LOGRADOURO,');
  Query.SQL.Add('  NUM_ENDERECO    = :pNUM_ENDERECO,');
  Query.SQL.Add('  NOM_BAIRRO      = :pNOM_BAIRRO,');
  Query.SQL.Add('  NOM_CIDADE      = :pNOM_CIDADE,');
  Query.SQL.Add('  SIG_UF          = :pSIG_UF,');
  Query.SQL.Add('  END_COMPLEMENTO = :pEND_COMPLEMENTO,');
  Query.SQL.Add('  END_ELETRONICO  = :pEND_ELETRONICO,');
  Query.SQL.Add('  TEL_FIXO        = :pTEL_FIXO,');
  Query.SQL.Add('  TEL_FAX         = :pTEL_FAX,');
  Query.SQL.Add('  TEL_CELULAR     = :pTEL_CELULAR,');
  ...
  Query.SQL.Add('WHERE');
  Query.SQL.Add('  COD_CLIENTE = :pCOD_CLIENTE');
